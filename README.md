...
The transformation for a random compound in the dataset would look like the following:

<p align="center">
  <img src="img/zinc_slrc_transform.png" width=75% alt="ZINC example">
</p>
...
## Discussion
Our experiment with selective long-range connections in Graph Neural Networks (GNNs) has shown promising results in addressing the challenge of over-squashing. The significant accuracy improvements observed in the TreeNeighborsMatch task, where over-squashing is a known issue, highlight the effectiveness of this approach. Both the Graph Convolutional Network (GCN) and Graph Isomorphism Network (GIN) models demonstrated high accuracy, indicating the potential of selective long-range connections in managing complex graph-structured data.

However, the real-world applicability of this architecture varies. For instance, in tasks like ZINC, which primarily require local information aggregation, our architecture didn't show improvement, likely due to minimal over-squashing challenges. Conversely, the QM9 task, which demands global message transmission, benefited from our approach, underscoring its utility in scenarios requiring extensive information exchange.

This underscores the complexity of effectively employing this architecture and suggests its suitability for specific tasks, particularly those necessitating broad information exchange. While the outcomes are promising, a more extensive exploration could enrich our understanding. Testing beyond a single depth (in this case, depth 4) and experimenting with various edge selection strategies could provide deeper insights into the nuanced impacts of selective long-range connections across different complexities and graph sizes.
